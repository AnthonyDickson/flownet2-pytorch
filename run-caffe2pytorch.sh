...
sudo docker build -t $USER/pytorch:CUDA8-py27 .
sudo docker run --rm -ti --gpus all --volume=${FN2PYTORCH}:/flownet2-pytorch:rw --workdir=/flownet2-pytorch $USER/pytorch:CUDA8-py27 /bin/bash -c "./install.sh"

# convert FlowNet2-C, CS, CSS, CSS-ft-sd, SD, S and 2 to PyTorch
sudo docker run -ti --gpus all --volume=${FN2PYTORCH}:/fn2pytorch:rw flownet2:latest /bin/bash -c "source /flownet2/flownet2/set-env.sh && cd /flownet2/flownet2/models && \
